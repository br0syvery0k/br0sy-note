    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.resultInfo.code == 0) {
                ...
                exit();
                // 操作DOM
                addDOM(lastFakeId, typeName, data.resultInfo.typeId);
                // 提示用户
                alert("类型添加成功！");
...
function exit() {
    ...
    // 数据清零
    $('#inputTypeId').prop("value", "");
}

// 添加类型DOM操作
function addDOM(lastFakeId, typeName, typeId) {
// 操作前端DOM
    console.log(typeof(typeName));
    var ID = 1;
    if (isNumber(lastFakeId)) {
        ID = parseInt(lastFakeId) + 1;
    } else {
        // 取消回显
        $("#emptyNote").css("display", "none");
    }
    var tr = '<tr><td>'+ID.toString()+'</td><td>'+typeName+'</td><td>';
    // 注意下面的typeName边上的两个转义的 \' 这个不加的话，在前端会解析成一个变量
    tr += '<button onClick="borderBox(\''+typeName+'\', '+typeId+', '+ID.toString()+')">修改</button>';
    tr += '&nbsp<button  onclick="deleteBox('+typeId+')">删除</button></td></tr>';
